...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 4.1. Se o cupom veio da fidelidade, marcar a recompensa como resgatada
    if (cupom.origem === "fidelidade" && cupom.nome) {
      ...
      if (resgateError) {
        console.error("[registrar-uso-cupom] Erro ao marcar recompensa como resgatada:", resgateError);
      } else {
        console.log("[registrar-uso-cupom] Recompensa marcada como resgatada:", cupom.nome);
      }
    }
