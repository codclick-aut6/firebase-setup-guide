...
const OrderDetails: React.FC<OrderDetailsProps> = ({ order, onUpdateStatus, onClose }) => {
  ...
  // 🟢 Código curto exibido ao admin — mesmo padrão usado em "Meus Pedidos" (primeiros 5 caracteres do ID)
  const shortCode = order?.id ? String(order.id).substring(0, 5) : null;
