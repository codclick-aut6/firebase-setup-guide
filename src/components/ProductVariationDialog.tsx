...
const ProductVariationDialog: React.FC<ProductVariationDialogProps> = ({
  ...
  const hasSizes = item.tipo === "pizza" && !isHalfPizza && (item.pizzaSizes?.length || 0) > 0;
              ...
              return sections.map((section, sIdx) => {
                ...
                return (
                  <div key={group.id} className="mb-6">
                    ...
                    {/* Descrição do grupo sempre visível (mesmo quando recolhido) */}
                    ...
                    {/* Grupo recolhível fechado: apenas o aviso */}
                    {isCollapsible && !isExpanded && (
                      <p className="text-xs text-muted-foreground text-center py-2">
                        Toque no título para ver todos os itens
                      </p>
                    )}

                    {/* Lista completa de variações quando não recolhível ou expandido */}
                    {(!isCollapsible || isExpanded) && (
