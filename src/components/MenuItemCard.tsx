  ...
  return (
    <>
        ...
        <div className={`flex items-center mt-2 ${isTwoColumns ? 'flex-col gap-2' : 'justify-between flex-row'}`}>
          <div className="flex flex-col items-start">
            ...
            {(() => {
              ...
              const displayPrice = hasSizes ? Math.min(...sizes.map((s) => s.price)) : item.price;
              const showFrom = item.priceFrom || hasSizes;
              return (
                <>
      ...
      <ProductVariationDialog
        ...
        onOpenPizzaCombination={(size) => { setPizzaCombinationSize(size ?? null); setIsPizzaDialogOpen(true); }}
      />
      <QuantityDialog
        item={item}
